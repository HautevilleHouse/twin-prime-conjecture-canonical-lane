#!/usr/bin/env python3
"""Canonical-lane closure guard for the twin prime conjecture workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['TP_G1', 'TP_G2', 'TP_G3', 'TP_G4', 'TP_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['TP_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['TP_GM'] = 'PASS' if gm_pass else 'FAIL'
    ...
    blockers['TP_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['TP_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    ...
    report = {
        'meta': {
            ...
            'framework': 'twin_prime_conjecture',
        },
        'schema': {
